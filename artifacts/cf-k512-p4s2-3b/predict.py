...
R=g.standard_normal((512,48),dtype=np.float32)*0.14433756729740646
t=g.standard_normal(512,dtype=np.float32)*0.1
def feats(x):
 n=len(x)
 ...
 h=np.maximum(c@R.T-t,0).reshape(n,15,15,512)
 return h.reshape(n,3,5,3,5,512).mean((2,4)).reshape(n,-1)
def predict(x):
