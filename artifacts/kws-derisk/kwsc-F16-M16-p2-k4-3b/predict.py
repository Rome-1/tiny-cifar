...
W=C[(np.unpackbits(np.frombuffer(B,np.uint8,72,40),bitorder="little")[:576].reshape(-1,3).astype(np.uint32)<<np.arange(3,dtype=np.uint32)).sum(1)].reshape(16,12)
...
def predict(x):
 ...
 h=np.maximum(c@R.T-t,0).reshape(len(x),2,7,2,7,4).mean((2,4)).reshape(len(x),-1)
