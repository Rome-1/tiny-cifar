...
W=C[(np.unpackbits(np.frombuffer(B,np.uint8,576,40),bitorder="little")[:4608].reshape(-1,3).astype(np.uint32)<<np.arange(3,dtype=np.uint32)).sum(1)].reshape(128,12)
g=np.random.default_rng(0)
R=g.standard_normal((32,9),dtype=np.float32)*9**-.5
t=g.standard_normal(32,dtype=np.float32)*.5
def predict(x):
 ...
 h=np.maximum(c@R.T-t,0).reshape(len(x),2,7,2,7,32).mean((2,4)).reshape(len(x),-1)
