...
W=C[(np.unpackbits(np.frombuffer(B,np.uint8,144,40),bitorder="little")[:1152].reshape(-1,3).astype(np.uint32)<<np.arange(3,dtype=np.uint32)).sum(1)].reshape(32,12)
...
def predict(x):
 ...
 h=np.maximum(c@R.T-t,0).reshape(len(x),2,7,2,7,8).mean((2,4)).reshape(len(x),-1)
