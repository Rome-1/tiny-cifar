...
b=B[0];k=1<<b;c=[16, 32, 48, 64]
C=np.frombuffer(B,np.float16,2*k,1).astype(np.float32).reshape(2,k)
S=np.frombuffer(B,np.float16,181,1+4*k).astype(np.float32)
q=np.unpackbits(np.frombuffer(B[1+4*k+2*181:],np.uint8),bitorder="little")[:7322*b].reshape(-1,b)
q=(q.astype(np.uint32)<<np.arange(b,dtype=np.uint32)).sum(1)
SP=[(0,c[0],(c[0],3,3,3)),(1,1,(c[0],))]
for i in range(3):SP+=[(0,1,(c[i],3,3)),(1,1,(c[i],)),(0,c[i+1],(c[i+1],c[i])),(1,1,(c[i+1],))]
SP+=[(0,10,(10,c[3])),(1,1,(10,))]
P=[];p=0;g=0
for u,m,s in SP:
