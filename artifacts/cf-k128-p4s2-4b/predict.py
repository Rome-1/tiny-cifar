...
R=g.standard_normal((128,48),dtype=np.float32)*0.14433756729740646
...
def feats(x):
 ...
 h=np.maximum(c@R.T-t,0).reshape(n,15,15,128)
 return h.reshape(n,3,5,3,5,128).mean((2,4)).reshape(n,-1)
def predict(x):
